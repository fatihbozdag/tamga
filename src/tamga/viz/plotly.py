"""Plotly (interactive) renderers -- parallel to `tamga.viz.mpl` for the most-used plot types.
...
Requires the optional `tamga[interactive]` extra (just `plotly>=5.20`).
"""
...
def _require_plotly() -> tuple:
    ...
    try:
        ...
        raise ImportError(
            "tamga.viz.plotly requires the optional `tamga[interactive]` extra "
            "(install with `pip install 'tamga[interactive]'` or `uv pip install plotly`)"
        ) from exc
    return (go,)
